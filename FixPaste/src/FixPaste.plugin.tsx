...
import { Patcher, DOM, getModule } from 'utils/BdApi';

import { SingletonListener } from 'utils/utils';

import { hasSubstrings } from 'utils/modules/filters';

const document_listener = SingletonListener(document);

export = class FixPaste {
    /**
    ...
    start() {
        ...
        if (assertElementModule) {
            ...
            Patcher.instead(assertElementModule, Object.keys(assertElementModule)[0], (_, [element, type]: any[], original: (e: any, t: any) => boolean) => {
        ...
        document_listener.setListener('paste', e => {
            ...
            if (3 === types.length && 'text/plain' === types[0] && 'text/html' === types[1] && clipboardData.files.length) {
                const getGoodFileName = (callback: (string) => void) => {
                        ...
                        Object.assign(
                            sliced_items[0],
                            {
                                getAsString: (callback => {
                                    ...
                                    getGoodFileName((name: string) => {
                                        callback(`<img src="https://example.com/images/${
                                            name
                                            .replaceAll(/[\s~]/g, '-')
                                            .replaceAll(/[/|]/g, '_')
                                            .replaceAll(/[-_]{3,}/g, '_')
                                ...
                                }) as DataTransferItem['getAsString']
                            }
                        ),
                    ];
                })();
