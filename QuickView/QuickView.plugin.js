...
module.exports = class QuickView {
    start() {
        ...
        const s = (className) => '.' + className?.split(' ').join('.');
            ...
            try { // POPOUT/MODAL BANNERS
                ...
                if (banner) {
                    ...
                    if (banner_el && !banner_el.quickview_patched) {
                        ...
                        banner_el.addEventListener('click', (e) => {
                            if (e.target !== banner_el)
                                return;
                            openModal(banner_url.slice(0, banner_url.indexOf('?')) + '?size=4096', 4096, 1638, banner_url);
                        });
                ...
                if (avatar) {
                    ...
                    if (avatar_el && !avatar_el.quickview_patched) {
                        ...
                        avatar_el.addEventListener('click', (e) => {
                            if (!is_modal && !e.altKey)
                                return;
                            e.stopPropagation();
                            openModal(avatar_url.slice(0, avatar_url.indexOf('?')) + '?size=4096', 4096, 4096, avatar_url);
                        });
                ...
                try { // SERVER BANNERS
                    ...
                    if (banner_container_el && !banner_container_el.quickview_patched) {
                        banner_container_el.addEventListener('click', (e) => {
                            ...
                            e.stopPropagation();
                            openModal(banner_url.slice(0, banner_url.indexOf('?')) + '?size=4096', img.naturalWidth, img.naturalHeight, banner_url);
                        });
                ...
                try { // DMS LIST AVATARS
                    ...
                    props.onClick = (e) => {
                        try {
                            if (!e.altKey)
                                return o_onClick(e);
                            const img = walkTree(e.target, e => e instanceof HTMLImageElement, ['firstElementChild']);
                            if (!img)
                                return o_onClick(e);
                            const avatar_url = img.currentSrc;
                            e.stopPropagation();
                            e.preventDefault();
                            openModal(img.currentSrc.slice(0, img.currentSrc.indexOf('?')) + '?size=4096', 4096, 4096, avatar_url);
                            return;
                        }
                        catch (e) {
            ...
            Patcher.before('QuickView', PopoutHandler.prototype, 'render', (that) => {
                try {
                    ...
                    if (message_contents_el) {
                        ...
                        try { // ROLE ICONS
                            const img = message_contents_el.querySelector('h2 img');
                            if (!img)
                                return;
                            ...
                            img.addEventListener('click', (e) => {
                                ...
                                e.stopPropagation();
                                e.preventDefault();
                                openModal(full_size, img.naturalWidth, img.naturalHeight, icon_url);
                            });
                        }
                        catch (e) {
                    ...
                    if (dom_node) { // GENERIC / MEMBER LIST AVATARS
                        const avatar_wrapper_el = dom_node.querySelector('[class^="avatar-"]');
                        if (!avatar_wrapper_el)
                            return;
                        const img = avatar_wrapper_el.querySelector('img[class^="avatar-"]');
                        if (!img)
                            return;
                        const avatar_url = img.currentSrc;
                        if (!avatar_url)
                            return;
                        avatar_wrapper_el.addEventListener('click', (e) => {
                            if (!e.altKey)
                                return;
                            e.stopPropagation();
                            e.preventDefault();
                ...
                try { // EMBED AUTHOR ICON
                    return (BdApi.React.createElement("span", { onClick: (e) => {
                            try {
                                ...
                                e.stopPropagation();
                                e.preventDefault();
                                openModal(icon_url, img.naturalWidth, img.naturalHeight, icon_url);
                            }
                            catch (e) {
                ...
                try { // EMOJIS
                    Patcher.after('QuickView', that.props, 'onClick', (_, [e]) => {
                        try {
                            ...
                            e.stopPropagation();
                            e.preventDefault();
                            openModal(full_size, img.naturalWidth, img.naturalHeight, emoji_url);
                        }
                        catch (e) {
    ...
    guildsListListener(e) {
        ...
        try {
            ...
            e.stopPropagation();
            openModal(icon_url.slice(0, icon_url.indexOf('?')) + '?size=4096', 4096, 4096, icon_url);
        }
        catch (e) {
